...
parser.add_argument('--dmax', type=float, default=0.01, help='Final time in days.')
parser.add_argument('--dumpt', type=float, default=6, help='Dump time in hours.')
...
qn = fd.Function(pvcalc(un).function_space(), name="Relative Vorticity")
...
stepcount = 0
while t < tmax + 0.5*dt:
    PETSc.Sys.Print(f"\nTime: {t/units.hour} | Iteration: {stepcount}\n")
    t += dt
    tdump += dt
